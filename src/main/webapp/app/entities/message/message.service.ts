...
export class MessageService {
    ...
    private convertDateFromClient(message: IMessage): IMessage {
        const copy: IMessage = Object.assign({}, message, {
            sentDate: message.sentDate != null && message.sentDate.isValid() ? message.sentDate.toJSON() : null
        });
        return copy;
    }
    ...
    private convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        res.body.forEach((message: IMessage) => {
            message.sentDate = message.sentDate != null ? moment(message.sentDate) : null;
        });
        return res;
    }
}
